...
二． 代码实现
（1） ADS第一次返回的报文：
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/1.png" alt="图片描述">
...
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/2.png" alt="图片描述">
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/3.png" alt="图片描述">
（2） ADS第二次返回的报文如下：
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/4.png" alt="图片描述">
在第二次向ads发送报文才是取值的操作。While循环中只有满足以下条件才会跳出：
①pauseFlag的值为false时。
②与ADS信息交互通讯超时。
③从ADS成功拿到了返回值。
从ADS拿到的报文中，关键字“Num”表示返回的结果条数（即我们真正想要的数据），如果返回条数大于等于1条，只需取第一条返回的结果数据即可。
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/5.png" alt="图片描述">
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/6.png" alt="图片描述">
跳出while循环，则关闭窗口，并第三次向ADS发送报文，请求释放掉DLL资源。
（3） ADS第三次返回的报文：
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/7.png" alt="图片描述">
返回的结果集里面，Code的值为0，则表明 ADS释放DLL资源成功。
三． 流程图
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/8.png" alt="图片描述">
四． 例子
① 在流程图中，添加组件 “外设操作类组件”，入参和出参的配置如下图：
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/9.png" alt="图片描述">
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/10.png" alt="图片描述">
在交易页面中，通过点击按钮来执行此lfc流程。
② 部署更新资源，在ABC客户端点击按钮，弹出对话框。如图：
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/11.png" alt="图片描述">
③ 调用外设，在一定时间内不做刷卡操作，返回的最终结果如下：
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/12.png" alt="图片描述">
④ 调用外设，在外设的超时时间内，做刷卡操作，返回的最终结果如下：
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/13.png" alt="图片描述">
调用外设，按中断热键或Enter键，执行操作操作，返回的最终结果如下：
<img src="@source/AB产品知识库20240403/images/平台/AB4/外设/AB4.0增加中断，取消功能-广东农信/14.png" alt="图片描述">
